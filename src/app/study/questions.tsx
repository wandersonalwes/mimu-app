  ...
  return (
    <SafeAreaView style={{ flex: 1 }} edges={['bottom']}>
      ...
      <View
        ...
        accessibilityLabel="Perguntas"
        testID="questions-screen"
      >
        {finished ? (
          <View className="flex-1 px-5 gap-6 justify-center">
            <View className="bg-card rounded-xl p-8 gap-6">
              <View className="items-center gap-2">
                ...
                <Text className="text-foreground/60 text-base font-manrope-regular text-center">
              ...
              <View className="bg-background rounded-xl p-4">
                <View className="flex-row items-center justify-center gap-2 mb-4">
                  <Text className="text-foreground text-4xl font-manrope-bold">{successRate}%</Text>
                </View>
                ...
                <View className="gap-3">
                  <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center gap-2">
                      ...
                      <Text className="text-foreground text-sm font-manrope-regular">Acertos</Text>
                    </View>
                  ...
                  <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center gap-2">
                      ...
                      <Text className="text-foreground text-sm font-manrope-regular">Erros</Text>
                    </View>
                    <Text className="text-foreground text-lg font-manrope-bold">{wrongCount}</Text>
                  </View>

                  <View className="h-px bg-border my-1" />

                  <View className="flex-row items-center justify-between">
                    <Text className="text-foreground text-sm font-manrope-semibold">Total</Text>
                    <Text className="text-foreground text-lg font-manrope-bold">
                      {questions.length}
                    </Text>
                  </View>
                ...
                return (
                  <TouchableOpacity
                    ...
                    className={`px-5 py-3.5 rounded-xl flex-row items-center ${
                      isCorrect ? 'bg-green-600/20' : isWrong ? 'bg-red-600/20' : 'bg-card'
                    }`}
                  ...
                  >
                    <Text
                      className={`text-sm font-manrope-regular flex-1 ${
                        isCorrect ? 'text-green-600' : isWrong ? 'text-red-600' : 'text-foreground'
                      }`}
                    >
                      {opt}
                    </Text>
